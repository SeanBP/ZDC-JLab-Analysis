...
def add_energy_columns(df):
    ...
    df["energy_GeV_full"] = (
        df["energy_MIP_full"] * gev_MIP * sampling * calibration
    )

    df["energy_GeV_avg"] = (
        df["energy_MIP_avg"] * gev_MIP * sampling * calibration
    )
...
for run in run_folders:
    ...
    pkl_files = sorted(
        glob.glob(os.path.join(run_path, "*_calibrated.pkl"))
    )
    ...
    for i, pkl in enumerate(pkl_files, start=1):
        ...
        # -------------------------------------------------
        # Hit multiplicity per event (IDENTICAL TO REFERENCE SCRIPT)
        # -------------------------------------------------
        ...
        hit_mult = hit_mult.reindex(
            cog_full.index,
            fill_value=0
        )

        # -------------------------------------------------
        # Combined event cut (IDENTICAL LOGIC)
        # -------------------------------------------------
        event_mask = (
        ...
        for tag in energy_tags:
            ...
            event_summary = event_summary.join(
                merged,
                how="left"
            )
